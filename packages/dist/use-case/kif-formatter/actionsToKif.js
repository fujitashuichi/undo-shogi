...
import { PromotablePieceKindSchema, PromotedPieceKindSchema } from "../../core/entities/types/piece.types.js";
...
export const actionsToKif = (moves) => {
    let kifLines = ["手数----指手"];
    let lastTo = null;
    moves.forEach((move, i) => {
        ...
        if (move.type === "drop") {
            ...
            let promoteStr = "";
            const side = step % 2 === 1 ? "Sente" : "Gote";
            if (!isAlreadyPromoted) {
                if (move.promote) {
                    promoteStr = "成";
                }
                else {
                    const canPromote = PromotablePieceKindSchema.safeParse(baseKind).success && (isInPromotionZone(side, fromPosition) ||
...
//# sourceMappingURL=actionsToKif.js.map