...
describe("legalDropPositions", () => {
    describe("二歩", () => {
        it("二歩は選択肢に出ない", () => {
            ...
            expect(legalDropPositions.byPiece(board, fullHands, "Pawn", "Sente")).toStrictEqual([]);
        });
    });
    describe("王手関連", () => {
        it("王手放置で持ち駒を打てない", () => {
            const board = new Board(checkedSquares);
            expect(legalDropPositions.all(board, fullHands, "Sente")).toStrictEqual([]);
        });
        describe("王手を遮る形で持ち駒を打てる", () => {
            it("飛車による王手の例", () => {
                ...
                vitest_checkArray.containingEachOther(legalDropPositions.byPiece(board, fullHands, "Gold", "Sente"), expected);
            });
            it("角による王手の例", () => {
    ...
    describe("打ち歩詰め", () => {
        it("打ち歩詰めは選択肢に出ない", () => {
            ...
            expect(legalDropPositions.byPiece(board, fullHands, "Pawn", "Sente")).not.toContainEqual([
                { x: 8, y: 1 }
            ]);
        });
    });
});
//# sourceMappingURL=legalDropPositions.test.js.map