...
describe("generateLegalMoves", () => {
    describe(".byPiece() で、合法手を正しく取得する", () => {
        it("通常局面", () => {
            ...
            vitest_checkArray.containingEachOther(legalMovePositions.byPiece(board, { x: 1, y: 1 }), [
        ...
        it("ピンされた局面（間接王手飛車）はピン駒（飛車）の有効な移動がない", () => {
            ...
            expect(legalMovePositions.byPiece(board, { x: 3, y: 7 })).toStrictEqual([]);
        });
    });
    describe(".all() で、合法手を正しく取得する", () => {
        it("通常局面", () => {
            ...
            let expected = [];
            for (let i = 0; i < 9; i++) {
                expected.push({ x: i, y: 5 });
            }
            expected.push({ x: 0, y: 7 }, { x: 2, y: 7 }, { x: 3, y: 7 }, { x: 4, y: 7 }, { x: 5, y: 7 }, { x: 6, y: 7 }, { x: 8, y: 7 });
            vitest_checkArray.containingEachOther(legalMovePositions.all(board, "Sente"), expected);
        });
        it("ピンされた局面（間接王手飛車）はピン駒（飛車）の有効な移動がない", () => {
            const board = new Board(pinnedRookSquares);
            const expected = [
...
//# sourceMappingURL=legalMovePositions.test.js.map