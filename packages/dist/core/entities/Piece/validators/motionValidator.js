...
const assertMotionVector = (board, current, next) => {
    ...
    if (!isValid)
        throw new PieceError("INVALID_MOTION_VECTOR");
};
const violatesLeapRestriction = (board, current, next) => {
    const piece = board.squares[current.y][current.x];
    if (!piece)
        throw new MovementError("MOVE_UNDEFINED_PIECE");
    const vectors = piece.motion.vectors;
    for (const vector of vectors) {
        ...
        if (vector.infinity) {
            while (isInBoard(x, y)) {
                ...
                if (next.x === x && next.y === y) {
                    if (collided)
                        throw new PieceError("LEAP_RESTRICTION");
                    return;
                }
...
const assertForcedPromotion = (board, current, next, promote) => {
    if (promote)
        return;
    const piece = board.squares[current.y][current.x];
    if (!piece)
    ...
    const endOfBoardYRangeTwo = side === "Sente" ? [0, 1] : [boardSize - 1, boardSize - 2];
...
export const pieceMotionValidator = (board, current, next, promote) => {
...
//# sourceMappingURL=motionValidator.js.map