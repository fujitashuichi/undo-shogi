export const checkRepetition = (actionsHash) => {
    ...
    const candidates = Object.keys(hashPositions).filter(hash => hashPositions[hash] && hashPositions[hash].length >= 4);
    if (candidates.length === 0)
        return { isRepetition: false };
    const targetHash = candidates.reduce((earliest, current) => hashPositions[current][3] < hashPositions[earliest][3] ? current : earliest);
    if (!targetHash) {
        return {
            isRepetition: false
        };
